...
Public gblCurrentAdapter As String
Public gblNetworkTraffic As String

Public gblMaxSpeed  As String
Public gblMinSpeed  As String
...
Public Function fLicenceState() As Integer
    ...
    If fFExists(gblSettingsFile) Then ' does the tool's own settings.ini exist?
        slicence = fGetINISetting("Software\PzNetworkGauge", "licence", gblSettingsFile)
...
Public Sub mnuSupport_ClickEvent()
    ...
    If answer = vbYes Then
        Call ShellExecute(menuForm.hWnd, "Open", "https://github.com/yereverluvinunclebert/Panzer-Network-Gauge-" & gblCodingEnvironment & "/issues", vbNullString, App.Path, 1)
    End If
...
Public Sub readPrefsPosition()
    ...
    If gblDpiAwareness = "1" Then
        gblPrefsHighDpiXPosTwips = fGetINISetting("Software\PzNetworkGauge", "formHighDpiXPosTwips", gblSettingsFile)
        gblPrefsHighDpiYPosTwips = fGetINISetting("Software\PzNetworkGauge", "formHighDpiYPosTwips", gblSettingsFile)
    ...
    Else
        gblPrefsLowDpiXPosTwips = fGetINISetting("Software\PzNetworkGauge", "formLowDpiXPosTwips", gblSettingsFile)
        gblPrefsLowDpiYPosTwips = fGetINISetting("Software\PzNetworkGauge", "formLowDpiYPosTwips", gblSettingsFile)
...
Public Sub writePrefsPositionAndSize()
    ...
    If widgetPrefs.IsLoaded = True And widgetPrefs.WindowState = vbNormal Then ' when vbMinimised the value = -48000  !
        If gblDpiAwareness = "1" Then
            ...
            sPutINISetting "Software\PzNetworkGauge", "formHighDpiXPosTwips", gblPrefsHighDpiXPosTwips, gblSettingsFile
            sPutINISetting "Software\PzNetworkGauge", "formHighDpiYPosTwips", gblPrefsHighDpiYPosTwips, gblSettingsFile
        Else
            gblPrefsLowDpiXPosTwips = CStr(widgetPrefs.Left)
            gblPrefsLowDpiYPosTwips = CStr(widgetPrefs.Top)
            
            ' now write those params to the toolSettings.ini
            sPutINISetting "Software\PzNetworkGauge", "formLowDpiXPosTwips", gblPrefsLowDpiXPosTwips, gblSettingsFile
            sPutINISetting "Software\PzNetworkGauge", "formLowDpiYPosTwips", gblPrefsLowDpiYPosTwips, gblSettingsFile
...
Public Sub hardRestart()
    ...
    If fFExists(thisCommand) Then
        ...
        Call ShellExecute(widgetPrefs.hWnd, "open", thisCommand, "Panzer-Network-Gauge-VB6.exe prefs", "", 1)
    Else
...
'        Debug.Print ("Maximum Speed: " & gblMaxSpeed)
'        Debug.Print ("Minimum Speed: " & maxSpeedVArray(gblMinSpeed))
    Else
        strDBG = "check array"
