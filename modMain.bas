...
Public Sub mainRoutine(ByVal restart As Boolean)
    ...
    widgetName = "Panzer Network Gauge"
    thisPSDFullPath = App.path & "\Res\Panzer Network gauge VB6.psd"
    ...
    Call readSettingsFile("Software\PzNetworkGauge", gblSettingsFile)
...
Private Sub checkFirstTime()
   ...
   On Error GoTo checkFirstTime_Error
    ...
    If gblFirstTimeRun = "true" Then
        ...
        sPutINISetting "Software\PzNetworkGauge", "firstTimeRun", gblFirstTimeRun, gblSettingsFile
    End If
...
'    gblSecondaryDaylightSaving = vbNullString
    ...
    gblMaxSpeed = vbNullString
    gblMinSpeed = vbNullString
...
'    overlayWidget.ZoomDirection = gblScrollWheelDirection
    ...
    If gblCurrentAdapter <> "" Then overlayWidget.thisAdapterName = panzerPrefs.cmbCurrentAdapter.List(Val(gblCurrentAdapter))
    
    'overlayWidget.thisNetworkNo = Val(gblCurrentAdapter)
...
Public Sub readSettingsFile(ByVal location As String, ByVal gblSettingsFile As String)
    ...
    If fFExists(gblSettingsFile) Then
        ...
        gblCurrentAdapter = fGetINISetting(location, "currentAdapter", gblSettingsFile)
        gblNetworkTraffic = fGetINISetting(location, "networkTraffic", gblSettingsFile)
...
'        gblClockFaceSwitchPref = fGetINISetting(location, "clockFaceSwitchPref", gblSettingsFile)
        ...
        ' other
        gblClockHighDpiXPos = fGetINISetting("Software\PzNetworkGauge", "clockHighDpiXPos", gblSettingsFile)
        gblClockHighDpiYPos = fGetINISetting("Software\PzNetworkGauge", "clockHighDpiYPos", gblSettingsFile)
        
        gblClockLowDpiXPos = fGetINISetting("Software\PzNetworkGauge", "clockLowDpiXPos", gblSettingsFile)
        gblClockLowDpiYPos = fGetINISetting("Software\PzNetworkGauge", "clockLowDpiYPos", gblSettingsFile)
        ...
        gblMaxSpeed = fGetINISetting(location, "maxSpeed", gblSettingsFile)
        gblMinSpeed = fGetINISetting(location, "minSpeed", gblSettingsFile)
...
'        If gblAnimationInterval = vbNullString Then gblAnimationInterval = "130"
        ...
        If gblCurrentAdapter = vbNullString Then gblCurrentAdapter = "0"
        If gblNetworkTraffic = vbNullString Then gblNetworkTraffic = "0"
        ...
        If gblDblClickCommand = vbNullString Then gblDblClickCommand = "%systemroot%\system32\ncpa.cpl"
        ...
        If gblMaxSpeed = vbNullString Then gblMaxSpeed = "0"
        If gblMinSpeed = vbNullString Then gblMinSpeed = "0"
...
Private Sub getToolSettingsFile()
    ...
    gblSettingsDir = fSpecialFolder(feUserAppData) & "\PzNetworkGauge" ' just for this user alone
...
Private Sub handleUnhideMode(ByVal thisUnhideMode As String)
    ...
    If thisUnhideMode = "unhide" Then     'parse the command line
        gblUnhide = "true"
        sPutINISetting "Software\PzNetworkGauge", "unhide", gblUnhide, gblSettingsFile
