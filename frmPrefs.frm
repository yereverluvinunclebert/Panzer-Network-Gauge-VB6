...
Begin VB.Form widgetPrefs 
   Caption         =   "Panzer Network Gauge Preferences"
   ClientHeight    =   10545
   ClientLeft      =   60
   ClientTop       =   510
   ClientWidth     =   8880
   ...
   Begin VB.Frame fraGeneral 
      ...
      Begin VB.Frame fraGeneralInner 
         ...
         Begin VB.CheckBox chkGenStartup 
            Caption         =   "Run the Network Widget at Windows Startup "
            Height          =   465
            Left            =   1980
            TabIndex        =   170
         ...
         End
         Begin VB.Label lblGeneral 
            Caption         =   "Select the network traffic type to monitor. By default it will be both received and sent bytes."
            Height          =   660
            Index           =   5
            Left            =   2025
            TabIndex        =   176
...
Private pCmbNetworkTrafficBalloonTooltip As String
...
'---------------------------------------------------------------------------------------
' Procedure : btnLessCmbCurrentAdapter_Click
' Author    : beededea
' Date      : 28/06/2025
' Purpose   :
'---------------------------------------------------------------------------------------
...
Private Sub btnLessCmbCurrentAdapter_Click()
   On Error GoTo btnLessCmbCurrentAdapter_Click_Error
...
btnLessCmbCurrentAdapter_Click_Error:

    MsgBox "Error " & Err.Number & " (" & Err.Description & ") in procedure btnLessCmbCurrentAdapter_Click of Form widgetPrefs"
End Sub
...
Private Sub cmbCurrentAdapter_Click()
    ...
    gblCurrentAdapter = cmbCurrentAdapter.ListIndex
    sPutINISetting "Software\PzNetworkGauge", "currentAdapter", gblCurrentAdapter, gblSettingsFile
...
Private Sub cmbMaxSpeed_Click()
    ...
    If pvtPrefsStartupFlg = False Then ' don't run this on startup
        btnSave.Enabled = True ' enable the save button
        gblMaxSpeed = cmbMaxSpeed.List(cmbMaxSpeed.ListIndex)
...
Private Sub btnLessCmbCurrentAdapter_MouseMove(Button As Integer, Shift As Integer, x As Single, y As Single)
    If gblPrefsTooltips = "0" Then CreateToolTip btnLessCmbCurrentAdapter.hWnd, "click here to narrow the network adapter list", _
                  TTIconInfo, "Help on the narrowing the adapter field", , , , True
End Sub

Private Sub btnMoreCmbCurrentAdapter_MouseMove(Button As Integer, Shift As Integer, x As Single, y As Single)
    If gblPrefsTooltips = "0" Then CreateToolTip btnMoreCmbCurrentAdapter.hWnd, "click here to widen the network adapter list", _
                  TTIconInfo, "Help on the widening the adapter field", , , , True
End Sub
...
Private Sub optGaugeTooltips_Click(Index As Integer)
    ...
    If pvtPrefsStartupFlg = False Then
        ...
        sPutINISetting "Software\PzNetworkGauge", "gaugeTooltips", gblGaugeTooltips, gblSettingsFile
...
Private Sub optPrefsTooltips_Click(Index As Integer)
   ...
   On Error GoTo optPrefsTooltips_Click_Error
    ...
    If pvtPrefsStartupFlg = False Then
        ...
        sPutINISetting "Software\PzNetworkGauge", "prefsTooltips", gblPrefsTooltips, gblSettingsFile
   ...
   On Error GoTo cmbMultiMonitorResize_Click_Error
    ...
    If cmbMultiMonitorResize.ListIndex = 2 Then
        If prefsMonitorStruct.IsPrimary = True Then
            ...
            sPutINISetting "Software\PzNetworkGauge", "gaugePrimaryHeightRatio", gblGaugePrimaryHeightRatio, gblSettingsFile
            
            'gblPrefsPrimaryHeightTwips = Trim$(cstr(widgetPrefs.Height))
            sPutINISetting "Software\PzNetworkGauge", "prefsPrimaryHeightTwips", gblPrefsPrimaryHeightTwips, gblSettingsFile
        Else
            gblGaugeSecondaryHeightRatio = fGauge.gaugeForm.WidgetRoot.Zoom
            sPutINISetting "Software\PzNetworkGauge", "gaugeSecondaryHeightRatio", gblGaugeSecondaryHeightRatio, gblSettingsFile
            
            'gblPrefsSecondaryHeightTwips = Trim$(cstr(widgetPrefs.Height))
            sPutINISetting "Software\PzNetworkGauge", "prefsSecondaryHeightTwips", gblPrefsSecondaryHeightTwips, gblSettingsFile
        End If
    End If
...
Private Sub Form_Load()
    ...
    Me.mnuAbout.Caption = "About Panzer Network Gauge Cairo " & gblCodingEnvironment & " widget"
...
Private Sub initialisePrefsVars()
   ...
   On Error GoTo initialisePrefsVars_Error
    ...
    pCmbNetworkTrafficBalloonTooltip = vbNullString
...
Private Sub subClassControls()
   ...
   On Error GoTo subClassControls_Error
    ...
    Else
        ...
        Call SubclassComboBox(cmbCurrentAdapter.hWnd, ObjPtr(cmbCurrentAdapter))
        Call SubclassComboBox(cmbNetworkTraffic.hWnd, ObjPtr(cmbNetworkTraffic))
        Call SubclassComboBox(cmbMaxSpeed.hWnd, ObjPtr(cmbMaxSpeed))
...
Public Sub MouseMoveOnComboText(sComboName As String)
    ...
    Select Case sComboName
        ...
        Case "cmbNetworkTraffic"
            sTitle = "Help on Debug Mode"
            sText = pCmbNetworkTrafficBalloonTooltip
            If gblPrefsTooltips = "0" Then CreateToolTip cmbNetworkTraffic.hWnd, sText, , sTitle, , , , True
        Case "cmbMaxSpeed"
            sTitle = "Help on Debug Mode"
...
Private Sub chkDpiAwareness_Click()
    ...
    If pvtPrefsStartupFlg = False Then ' don't run this on startup
        ...
        sPutINISetting "Software\PzNetworkGauge", "dpiAwareness", gblDpiAwareness, gblSettingsFile
...
Private Sub cmbDebug_Click()
    ...
    If cmbDebug.ListIndex = 0 Then
        txtDefaultEditor.Text = "eg. E:\vb6\Panzer-Network-Gauge-VB6MkII\Panzer-Network-Gauge-VB6.vbp"
...
Private Sub btnSave_Click()
    ...
    gblCurrentAdapter = LTrim$(Str$(cmbCurrentAdapter.ListIndex))
    gblNetworkTraffic = LTrim$(Str$(cmbNetworkTraffic.ListIndex))
...
'    End If
    ...
    If fFExists(gblSettingsFile) Then
        ...
        sPutINISetting "Software\PzNetworkGauge", "showTaskbar", gblShowTaskbar, gblSettingsFile
        sPutINISetting "Software\PzNetworkGauge", "showHelp", gblShowHelp, gblSettingsFile
        
        sPutINISetting "Software\PzNetworkGauge", "dpiAwareness", gblDpiAwareness, gblSettingsFile
        
        
        sPutINISetting "Software\PzNetworkGauge", "gaugeSize", gblGaugeSize, gblSettingsFile
        sPutINISetting "Software\PzNetworkGauge", "scrollWheelDirection", gblScrollWheelDirection, gblSettingsFile
                
        sPutINISetting "Software\PzNetworkGauge", "widgetFunctions", gblGaugeFunctions, gblSettingsFile
        
        sPutINISetting "Software\PzNetworkGauge", "pointerAnimate", gblPointerAnimate, gblSettingsFile
        sPutINISetting "Software\PzNetworkGauge", "samplingInterval", gblSamplingInterval, gblSettingsFile
        
        sPutINISetting "Software\PzNetworkGauge", "maxSpeed", gblMaxSpeed, gblSettingsFile
        sPutINISetting "Software\PzNetworkGauge", "minSpeed", gblMinSpeed, gblSettingsFile
        
        sPutINISetting "Software\PzNetworkGauge", "currentAdapter", gblCurrentAdapter, gblSettingsFile
        sPutINISetting "Software\PzNetworkGauge", "networkTraffic", gblNetworkTraffic, gblSettingsFile
        
              
        sPutINISetting "Software\PzNetworkGauge", "aspectHidden", gblAspectHidden, gblSettingsFile
        sPutINISetting "Software\PzNetworkGauge", "widgetPosition", gblWidgetPosition, gblSettingsFile
        sPutINISetting "Software\PzNetworkGauge", "widgetLandscape", gblWidgetLandscape, gblSettingsFile
        sPutINISetting "Software\PzNetworkGauge", "widgetPortrait", gblWidgetPortrait, gblSettingsFile

        sPutINISetting "Software\PzNetworkGauge", "prefsFont", gblPrefsFont, gblSettingsFile
        sPutINISetting "Software\PzNetworkGauge", "gaugeFont", gblGaugeFont, gblSettingsFile
        
        sPutINISetting "Software\PzNetworkGauge", "prefsFontSizeHighDPI", gblPrefsFontSizeHighDPI, gblSettingsFile
        sPutINISetting "Software\PzNetworkGauge", "prefsFontSizeLowDPI", gblPrefsFontSizeLowDPI, gblSettingsFile
        sPutINISetting "Software\PzNetworkGauge", "prefsFontItalics", gblPrefsFontItalics, gblSettingsFile
        sPutINISetting "Software\PzNetworkGauge", "prefsFontColour", gblPrefsFontColour, gblSettingsFile
        
        sPutINISetting "Software\PzNetworkGauge", "displayScreenFont", gblDisplayScreenFont, gblSettingsFile
        sPutINISetting "Software\PzNetworkGauge", "displayScreenFontSize", gblDisplayScreenFontSize, gblSettingsFile
        sPutINISetting "Software\PzNetworkGauge", "displayScreenFontItalics", gblDisplayScreenFontItalics, gblSettingsFile
        sPutINISetting "Software\PzNetworkGauge", "displayScreenFontColour", gblDisplayScreenFontColour, gblSettingsFile

        'save the values from the Windows Config Items
        sPutINISetting "Software\PzNetworkGauge", "windowLevel", gblWindowLevel, gblSettingsFile
        sPutINISetting "Software\PzNetworkGauge", "preventDragging", gblPreventDragging, gblSettingsFile
        
        sPutINISetting "Software\PzNetworkGauge", "opacity", gblOpacity, gblSettingsFile
        sPutINISetting "Software\PzNetworkGauge", "widgetHidden", gblWidgetHidden, gblSettingsFile
        sPutINISetting "Software\PzNetworkGauge", "hidingTime", gblHidingTime, gblSettingsFile
        sPutINISetting "Software\PzNetworkGauge", "ignoreMouse", gblIgnoreMouse, gblSettingsFile
        sPutINISetting "Software\PzNetworkGauge", "multiMonitorResize", gblMultiMonitorResize, gblSettingsFile
        
        
        sPutINISetting "Software\PzNetworkGauge", "startup", gblStartup, gblSettingsFile

        sPutINISetting "Software\PzNetworkGauge", "enableSounds", gblEnableSounds, gblSettingsFile
'        sPutINISetting "Software\PzNetworkGauge", "enableTicks", gblEnableTicks, gblSettingsFile
'        sPutINISetting "Software\PzNetworkGauge", "enableChimes", gblEnableChimes, gblSettingsFile
'        sPutINISetting "Software\PzNetworkGauge", "enableAlarms", gblEnableAlarms, gblSettingsFile
'        sPutINISetting "Software\PzNetworkGauge", "volumeBoost", gblVolumeBoost, gblSettingsFile
        
        sPutINISetting "Software\PzNetworkGauge", "lastSelectedTab", gblLastSelectedTab, gblSettingsFile
        
        sPutINISetting "Software\PzNetworkGauge", "debug", gblDebug, gblSettingsFile
        sPutINISetting "Software\PzNetworkGauge", "dblClickCommand", gblDblClickCommand, gblSettingsFile
        sPutINISetting "Software\PzNetworkGauge", "openFile", gblOpenFile, gblSettingsFile
        sPutINISetting "Software\PzNetworkGauge", "defaultVB6Editor", gblDefaultVB6Editor, gblSettingsFile
        sPutINISetting "Software\PzNetworkGauge", "defaultTBEditor", gblDefaultTBEditor, gblSettingsFile
        
        sPutINISetting "Software\PzNetworkGauge", "gaugeHighDpiXPos", gblGaugeHighDpiXPos, gblSettingsFile
        sPutINISetting "Software\PzNetworkGauge", "gaugeHighDpiYPos", gblGaugeHighDpiYPos, gblSettingsFile
        
        sPutINISetting "Software\PzNetworkGauge", "gaugeLowDpiXPos", gblGaugeLowDpiXPos, gblSettingsFile
        sPutINISetting "Software\PzNetworkGauge", "gaugeLowDpiYPos", gblGaugeLowDpiYPos, gblSettingsFile
...
Private Sub adjustPrefsControls(Optional ByVal restartState As Boolean)
    ...
    cmbCurrentAdapter.ListIndex = Val(gblCurrentAdapter)
    cmbNetworkTraffic.ListIndex = Val(gblNetworkTraffic)
    
    For I = 0 To cmbMaxSpeed.ListCount
        If gblMaxSpeed = cmbMaxSpeed.List(I) Then
...
'    chkVolumeBoost.Value = Val(gblVolumeBoost)
    ...
    lblGitHub.Caption = "You can find the code for the Panzer Network Gauge on github, visit by double-clicking this link https://github.com/yereverluvinunclebert/Panzer-Network-Gauge"
...
'    cmbCurrentAdapter.ItemData(I) = 9999
    ...
    cmbNetworkTraffic.AddItem "Received", 1
    cmbNetworkTraffic.ItemData(1) = 1
    cmbNetworkTraffic.AddItem "Sent", 2
    cmbNetworkTraffic.ItemData(2) = 2
    
    
    cmbMaxSpeed.AddItem "1", 0
    cmbMaxSpeed.ItemData(0) = 1
    cmbMaxSpeed.AddItem "2", 1
    cmbMaxSpeed.ItemData(1) = 2
    cmbMaxSpeed.AddItem "3", 2
    cmbMaxSpeed.ItemData(2) = 3
    cmbMaxSpeed.AddItem "4", 3
    cmbMaxSpeed.ItemData(3) = 4
    cmbMaxSpeed.AddItem "6", 4
    cmbMaxSpeed.ItemData(4) = 6
    cmbMaxSpeed.AddItem "8", 5
    cmbMaxSpeed.ItemData(5) = 8
    cmbMaxSpeed.AddItem "12", 6
    cmbMaxSpeed.ItemData(6) = 12
    cmbMaxSpeed.AddItem "16", 7
    cmbMaxSpeed.ItemData(7) = 16
    cmbMaxSpeed.AddItem "24", 8
    cmbMaxSpeed.ItemData(8) = 24
    cmbMaxSpeed.AddItem "32", 9
    cmbMaxSpeed.ItemData(9) = 32
    cmbMaxSpeed.AddItem "48", 10
    cmbMaxSpeed.ItemData(10) = 48
    cmbMaxSpeed.AddItem "64", 11
    cmbMaxSpeed.ItemData(11) = 64
    cmbMaxSpeed.AddItem "100", 12
    cmbMaxSpeed.ItemData(12) = 100
...
Private Sub lblGitHub_dblClick()
    ...
    If answer = vbYes Then
        Call ShellExecute(Me.hWnd, "Open", "https://github.com/yereverluvinunclebert/Panzer-Network-Gauge-" & gblCodingEnvironment, vbNullString, App.Path, 1)
    End If
...
Public Sub setPrefsTooltips()
   ...
   On Error GoTo setPrefsTooltips_Error
    ...
    If optPrefsTooltips(0).Value = True Then
        ...
        pCmbCurrentAdapterBalloonTooltip = "Identify your network adapter beforehand using the network panel on your system's settings or the control panel."
        pCmbNetworkTrafficBalloonTooltip = "By default both IN and OUT traffic will be monitored, you can select just received bytes input or those sent by the network adapter, selected above."
        pCmbMaxSpeedBalloonTooltip = "Estimate the estimated maximum speed of this network device (in Mbps). This setting will be adjusted automatically as the widget runs."
...
'    widgetPrefs.sliAnimationInterval.BackColor = RGB(redC, greenC, blueC)
    ...
    sPutINISetting "Software\PzNetworkGauge", "skinTheme", gblSkinTheme, gblSettingsFile
